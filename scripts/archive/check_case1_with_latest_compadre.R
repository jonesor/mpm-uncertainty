# Check case study 1 outputs using the latest local COMPADRE snapshot ----
#
# Usage:
# 1) Put latest COMPADRE file in data/raw/compadre/, e.g.
#    COMPADRE_6.25.8.0_Aug_14_2025.RData
# 2) source("scripts/check_case1_with_latest_compadre.R")
#
# This script:
# - backs up current active COMPADRE files
# - activates the latest local COMPADRE snapshot
# - reruns S01 + case-study-1 scripts (S02, S04-S10)
# - compares key case-study-1 CSV outputs to baseline
...
latest_file <- "data/raw/compadre/COMPADRE_v6.25.8.0.RData"
...
if (is.null(latest_file)) {
  ...
  if (length(latest_candidates) == 0) {
    stop(
      ...
      "Set `latest_file` manually in scripts/check_case1_with_latest_compadre.R."
    )
  }
...
backup_raw <- file.path(raw_dir, paste0("COMPADRE_v.X.X.X_pre_case1_check_", ts, ".RData"))
backup_corrected <- file.path(raw_dir, paste0("COMPADRE_v.X.X.X_Corrected_pre_case1_check_", ts, ".RData"))
...
compare_dir <- file.path("/tmp", paste0("case1_compare_", ts))
...
key_outputs <- c(
  "data/derived/analysis_cache/case1_beta_summary.csv",
...
cmp_rows <- purrr::map_dfr(key_outputs, function(f) {
  ...
  if (!file.exists(old_path) || !file.exists(new_path)) {
    return(tibble::tibble(file = f, rows_old = NA_integer_, rows_new = NA_integer_, status = "missing"))
  }
...
report_file <- file.path(compare_dir, "case1_latest_compadre_check.csv")
readr::write_csv(cmp_rows, report_file)

message("Case study 1 COMPADRE check complete.")
