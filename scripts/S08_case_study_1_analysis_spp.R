...
source("code/setup.R")
setup_packages(c(
  "tidyverse", "ggridges", "cowplot", "Rcompadre", "Rage",
...
p1 <- ggplot(sd_shape, aes(y = id_shape)) +
  ...
  geom_density_ridges(aes(x = shape),
    rel_min_height = 0.01,
...
p2 <- ggplot(sd_shape, aes(y = id_l0)) +
  ...
  coord_flip() +
  labs(
    y = expression(paste("Population (ranked by ", italic(L[alpha]), ")")),
...
p3 <- ggplot(sd_other, aes(y = id_growth)) +
  geom_density_ridges(aes(x = growth),
    rel_min_height = 0.01,
    scale = 2.5, fill = cols$fill, linewidth = 0.4
  ) +
  ...
  labs(
    y = expression(paste("Population (ranked by ", italic(gamma), ")")),
...
df_shape <- sd_shape %>%
  ...
  summarize(
    shape_med = quantile(shape, 0.500),
...
write_csv(
  df_shape %>% mutate(across(where(is.numeric), ~ signif(.x, 3))),
...
write_csv(
  df_other %>% mutate(across(where(is.numeric), ~ signif(.x, 3))),
