...
sd_satterthwaite <- satterthwaite %>%
  ...
  summarize(
    simU = list(list_mean(simU, na.rm = TRUE)),
...
andrello <- compadre %>%
  ...
  mutate(posF = list(mat_mean(matF) > 0)) %>%
  ungroup() %>%
  mutate(id = cdb_id_stages(., "MatrixClassOrganized"))
...
lemke <- compadre %>%
  ...
  filter(grepl("Overall summary matrix", Observation)) %>%
  cdb_unnest() %>%
  mutate(N = list(c(3435, 525, 703, 449, 318, 155))) %>%
  group_by(MatrixPopulation) %>%
  mutate(posU = list(mat_mean(matU) > 0)) %>%
  mutate(posF = list(mat_mean(matF) > 0)) %>%
  ungroup()
...
law_fec <- compadre %>%
  ...
  filter(MatrixComposite == "Mean") %>%
  cdb_unnest() %>%
  mutate(N = list(c(NA, NA, NA, NA, 74))) %>%
  mutate(posF = list(mat_mean(matF) > 0)) %>%
  mutate(simF = pmap(list(matF, posF, N), ~ sim_F_wrapper(..1, ..2, ..3, 1000)))
